 ...
 */
export const withRouterNext = <T extends {}>(
  Component: React.ComponentType<T & WithRouterNextProps>,
) => {
  return (props: Omit<T, keyof WithRouterNextProps>) => {
    const location = useLocation();
    const navigate = useNavigate();
    const params = useParams<RouterDOMParams>();

    // Combine the props with the router-specific props
    const componentProps = {
      ...props,
      params,
      location,
      navigate,
    } as T & WithRouterNextProps;
